...
const FLOW_TAB_URLS = ['https://labs.google/*'];
...
let latestCaptchaTokens = {};

chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg.type === 'PROACTIVE_CAPTCHA_TOKEN' && msg.token) {
    latestCaptchaTokens[msg.action || 'IMAGE_GENERATION'] = {
      token: msg.token,
      timestamp: Date.now(),
    };
    console.log('[Flow Agent] Received proactive captcha token for', msg.action);
  }
});

async function solveCaptcha(requestId, captchaAction) {
  const action = captchaAction || 'IMAGE_GENERATION';
  const cached = latestCaptchaTokens[action];
  if (cached && (Date.now() - cached.timestamp < 120000)) {
    console.log('[Flow Agent] Using fresh proactive captcha token for', action);
    return { token: cached.token };
  }

  try {
    const allTabs = await chrome.tabs.query({});
    const flowTab = allTabs.find(t => t.url && t.url.includes('labs.google'));
    const tab = flowTab || await getOrOpenFlowTab();
    ...
    try {
      const results = await chrome.scripting.executeScript({
        ...
        func: async (siteKey, action) => {
          try {
            ...
            return { error: 'grecaptcha not found on page' };
          } catch (err) {
            return { error: String(err) };
          }
        },
        args: [SITE_KEY, captchaAction || 'IMAGE_GENERATION']
      });
      ...
      if (results?.[0]?.result?.error) {
        console.warn('[Flow Agent] grecaptcha result warning:', results[0].result.error);
      }
    } catch (scriptErr) {
      console.warn('[Flow Agent] executeScript MAIN world error:', scriptErr);
    }

    // Fallback to tabs.sendMessage
    return await requestCaptchaFromTab(tab.id, requestId, captchaAction);
  } catch (e) {
