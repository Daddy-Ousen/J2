...
async function refreshCaptchaToken() {
  try {
    if (window.grecaptcha?.enterprise?.execute) {
      ...
      if (token) {
        window.dispatchEvent(new CustomEvent('PUSH_CAPTCHA_TOKEN', {
          detail: { action: 'IMAGE_GENERATION', token }
        }));
      }
    }
  } catch (err) {
...
setInterval(refreshCaptchaToken, 12000);
setTimeout(refreshCaptchaToken, 1500);
